...
| [Pipelines](./pipelines.md), [`schedule`](./commands/schedule.md) | Recent. `pipeline init` writes a pipeline file from a handful of questions (full-screen or line-by-line), so the YAML is optional. The surface may still gain steps — there are no conditionals yet. |
