...
**Interactive triage (shell only).** Inside the interactive shell, `/triage <path>` runs the same focus-aware analysis but doesn't make you wait: it auto-files the confident verdicts (clear sharp → `sharp/`, clear blurry → `blurry/`) immediately, then hands you *only* the uncertain shallow-DoF rescues one at a time. Each review card shows the scores, aperture, and a focus heatmap with a legend (soft → sharp) marking where focus landed; `K` keeps, `D` discards, `P` opens the frame in your system viewer, `S` skips, `Esc` finishes. Add `--dry-run` to walk the whole flow — analysis, review, decisions — without copying a single file. Still non-destructive — "discard" copies into `blurry/` for you to delete later. (Batch `shoots cull` stays non-interactive for scripting.)
